...
const processIncomingMessage = async (metadata, msg) => {
  ...
  switch (msg.type) {
    ...
    case 'image':   content = msg.image?.caption || ''; mediaUrl = msg.image?.id; break;
    case 'audio':   content = '[Audio message]'; mediaUrl = msg.audio?.id; break;
    case 'video':   content = msg.video?.caption || '[Video]'; mediaUrl = msg.video?.id; break;
    case 'document': content = msg.document?.filename || '[Document]'; mediaUrl = msg.document?.id; break;
    case 'location': content = `Location: ${msg.location?.latitude},${msg.location?.longitude}`; break;
    case 'button':  content = msg.button?.text || ''; break;
    case 'interactive': content = msg.interactive?.button_reply?.title || msg.interactive?.list_reply?.title || ''; break;
  ...
  if (msg.type === 'text' && content) {
    analyzeSentiment(content).then(async (sentiment) => {
      ...
      if (['frustrated', 'angry'].includes(sentiment)) {
        console.warn(`[Sentiment] 🚨 ${sentiment.toUpperCase()} customer: tenant=${tenant._id} contact=${contact._id}`);
      }
...
const processStatusUpdate = async (status) => {
  const update = {
    ...
    ...(status.timestamp && { [`${status.status}At`]: new Date(parseInt(status.timestamp) * 1000) }),
  };
